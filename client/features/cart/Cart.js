...
import { removeItemFromCart, updateCartItemQuantity } from '../../app/cartSlice'
import {
    getLocalStorageCart,
...
function Cart() {
    ...
    useEffect(() => {
        if (userId) {
            if (
                JSON.stringify(loggedInCartItems) !== JSON.stringify(cartItems)
            ) {
                dispatch(fetchCartItems(userId))
                setCartItems((prevCartItems) => {
                    if (
                        JSON.stringify(loggedInCartItems) !==
                    ...
                    <>
                        <table>
                            ...
                            <tbody>
                                {cartItems.map((item) => {
                                    const movie =
                                        userId && item.Movie ? item.Movie : item
                                    return (
                                        <tr key={item.id}>
